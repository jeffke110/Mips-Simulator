...
from test_config import TEST_FILE_DIR
...
class Test21(unittest.TestCase):
    ...
    def test_intial_status(self):
        """Testing initial status using /vm/test20.asm"""
        output = self.wrapper.run_command(u'status')
        self.assertEqual(output.strip(), "")
        output = self.wrapper.run_command(u'print $pc')
        self.assertEqual(output.strip(), "0x00000000")
    ...
    def test_step(self):
        """Testing run and break using /vm/test20.asm"""
        self.wrapper.run_command(u'run')
        time.sleep(1)
        self.wrapper.run_command(u'break')
